"""
Trains DQN, prediction characteristic, and Shapley models in parallel on the GWB environment
...
class ExpArgs:
    ...
    wandb_project_name: str = "FastSVERL-OffPolicy_UpdateRate_GWB"
...
class CharacteristicArgs:
    ...
    """whether the characteristic is off-policy"""
...
if __name__ == "__main__":
    ...
    for update_rate in [1, 2, 10, 50]:
        ...
        exp_args.group = f"{update_rate}_w_exact_{ExpArgs.group}"
        shapley_args.with_exact_char = True
        run()
        
        # Without exact char when computing exact loss
        exp_args.group = f"{update_rate}_wo_exact_{ExpArgs.group}"
