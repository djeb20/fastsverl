"""
Trains multiple outcome Shapley models for DQN agents in the GWB environment.
"""
...
if __name__ == "__main__":
    ...
    for seed in tqdm(range(1, exp_args.num_runs + 1), desc="Runs"):
        ...
        exp_args.group = f"w_policy_{ExpArgs.group}"
        processes.append(run())

        # Without exact policy char
        exp_args.char_args_f = None # Fill in wandb run name for characteristic here.
        exp_args.group = f"wo_policy_{ExpArgs.group}"
        processes.append(run())

        # ------ Without exact performance char ------
        shapley_args.with_exact_char = False
        exp_args.group = f"wo_perf_{ExpArgs.group}"
        processes.append(run())
