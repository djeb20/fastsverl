"""
Trains off-policy prediction characteristic models with different clipping coefficients in GWB. 
"""
...
class ExpArgs:
    ...
    wandb_project_name: str = "FastSVERL-OffPolicy_GWB_Clipping"
...
class CharacteristicArgs:
    ...
    """whether the characteristic is approximated off-policy or on-policy"""
    clip_coef: float = float('inf') # Change to 0.99, 0.995, 0.998 for different runs
    """the importance sampling ratio clipping coefficient"""
...
if __name__ == "__main__":
    ...
    # Buffered steady-state with importance sampling (no clipping)
    exp_args.group = f"w_weight_{ExpArgs.group}"
    char_args.off_policy = True
    run()

    # Buffered steady-state with importance sampling (clip = 0.99)
    exp_args.group = f"0.99_{ExpArgs.group}"
    char_args.off_policy = True
    char_args.clip_coef = 0.99
    run()

    # Buffered steady-state with importance sampling (clip = 0.995)
    exp_args.group = f"0.995_{ExpArgs.group}"
    char_args.off_policy = True
    char_args.clip_coef = 0.995
    run()

    # Buffered steady-state with importance sampling (clip = 0.998)
    exp_args.group = f"0.998_{ExpArgs.group}"
    char_args.off_policy = True
    char_args.clip_coef = 0.998
    run()