"""
Trains combined DQN, outcome characteristic and Shapley models on the GWB environment.
"""
...
@dataclass
class PolicyCharacteristicArgs(Verbose):
    save_model: bool = False
    """whether to save model into the `runs/{run_name}` folder"""
...
@dataclass
class PerformanceCharacteristicArgs(Verbose):
...
def run(agent):
    ...
    return subprocess.Popen([
        ...
        json.dumps(PolicyCharacteristicArgs().__dict__),
...
if __name__ == "__main__":
    ...
    for seed in tqdm(range(1, exp_args.num_runs + 1), desc="Runs"):
        ...
        # All separate
        exp_args.group = f"w_policy_dqn_{ExpArgs.group}"
        processes.append(run('DQN'))

        # Combined Shapley
        exp_args.group = f"w_policy_dqn_s_{ExpArgs.group}"
        ...
        # All separate
        exp_args.group = f"wo_policy_dqn_{ExpArgs.group}"
        processes.append(run('DQN'))

        # Combined Shapley
        exp_args.group = f"wo_policy_dqn_s_{ExpArgs.group}"
        processes.append(run('XDQN_S'))
        ...
        # All separate
        exp_args.group = f"wo_perf_dqn_{ExpArgs.group}"
        processes.append(run('DQN'))

        # Combined Shapley
        exp_args.group = f"wo_perf_dqn_s_{ExpArgs.group}"
        processes.append(run('XDQN_S'))
