"""
Trains combined DQN, behaviour characteristic and Shapley models on the Mastermind environment.
"""
...
if __name__ == "__main__":
    ...
    for seed in tqdm(range(1, exp_args.num_runs + 1), desc="Runs"):
        ...
        agent_args.exact_frequency = 5_000
        agent_args.exact_buffer_size = 10_000

        # Small policy characteristic
        policy_char_args.model_arch = {
            'hidden1': ['Linear', [64, 64]],
            'hidden1_activation': ['ReLU', []],
            'output': ['Linear', [64, None]],
            'output_activation': ['Softmax', [-1]],
        }
        policy_char_args.exact_loss = True

        # Small performance characteristic
        ...
        # All separate
        exp_args.group = f"222_w_policy_dqn_{ExpArgs.group}"
        processes.append(run('DQN'))

        # Combined Shapley
        exp_args.group = f"222_w_policy_dqn_s_{ExpArgs.group}"
        ...
        # All separate
        exp_args.group = f"222_wo_policy_dqn_{ExpArgs.group}"
        processes.append(run('DQN'))

        # Combined Shapley
        exp_args.group = f"222_wo_policy_dqn_s_{ExpArgs.group}"
        processes.append(run('XDQN_S'))
        ...
        # All separate
        exp_args.group = f"222_wo_perf_dqn_{ExpArgs.group}"
        processes.append(run('DQN'))

        # Combined Shapley
        exp_args.group = f"222_wo_perf_dqn_s_{ExpArgs.group}"
        processes.append(run('XDQN_S'))
